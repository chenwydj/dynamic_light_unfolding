...
class MSN(nn.Module):
    ...
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.final = nn.Conv2d(nclass+1, nclass, 1)
    ...
    def forward(self, image, Lab, seg, gray_mask):
        ...
        output_da = self.head(torch.cat([c4_image, c4_Lab, c4_seg], dim=1))
        gray_mask = F.upsample(gray_mask, size=(output_da.size(2), output_da.size(3)))
        output = self.final(torch.cat([output_da, gray_mask], dim=1))

        if self.aux and self.training:
            # auxout_image = self.head0(c4_image)
            # auxout_Lab = self.head1(c4_Lab)
            # auxout_seg = self.head2(c4_seg)
            return output#, output_da, auxout_image, auxout_Lab, auxout_seg
        else:
            return output
