...
class Policy(nn.Module):
    def __init__(self, hidden_dim=4, input_dim=3, rnn_type='lstm'):
        ...
        # self.resnet = resnet50(pretrained=True, num_classes=hidden_dim, dilated=True, norm_layer=nn.BatchNorm2d, root="./", multi_grid=False, multi_dilation=None)
        # self.fcn = FCN(hidden_dim, backbone='resnet50', root="./", dilated=True, norm_layer=nn.BatchNorm2d, multi_grid=False, multi_dilation=None, input_ch=input_dim)
    ...
    # def forward(self, x):
    def forward(self, image, Lab, seg, gray_mask):
        ...
        # x = self.fcn(x)[0]
        x = self.msn(image, Lab, seg, gray_mask)
