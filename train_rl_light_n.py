...
def train(epoch, reward_history, r_neg, r_pos):
    ...
    for i, data in enumerate(tbar):
        ...
        actions, dists, _ = policy(data["A"].cuda(), data["A_Lab"].cuda(), seg(data["A"].cuda())[0], data["gray_mask"].cuda())
        ...
        if i > 0 and (i + 1) % update_reward_interval == 0:
            ...
            if percent >= 0.55:
                ...
                if mode_action > 0: r_neg += delta; r_pos -= delta; r_pos = max(r_pos, 0)
                else: r_pos += delta; r_neg -= delta; r_neg = max(r_neg, 0)
            print("r_pos:", r_pos, "r_neg:", r_neg)
            total_dists = torch.empty(0, 4).cuda()
    ...
    with torch.no_grad():
        for i, data in enumerate(tbar):
            ...
            if mode == "origin":
                ...
                image = Image.open(data["A_paths"][0])
                mask = _mask_transform(Image.open(os.path.join("/ssd1/chenwy/bdd100k/seg/labels/", "train", os.path.splitext(data["A_paths"][0].split("/")[-1])[0] + '_train_id.png')))
                ...
                for i in range(n_w):
                    for j in range(n_h):
                        # x is w, y is h
                        x = int(np.round(i * step_w)); y = int(np.round(j * step_h))
                        A = transformer(image.crop((x, y, x+crop_size, y+crop_size))).unsqueeze(0).cuda()
                        r,g,b = A[0, 0, :, :]+1, A[0, 1, :, :]+1, A[0, 2, :, :]+1
                        A_gray = 1. - (0.299*r+0.587*g+0.114*b)/2. # h, w
...
if evaluation:
    ...
    opt_seg.checkname = "image.Lab.seg.gray_0.75_res50.act4_adapt20.0.1.clip5.5_entropy0.3_lr3e3.800epoch_" + time.strftime("%Y-%m-%d-%H-%M",time.localtime())
